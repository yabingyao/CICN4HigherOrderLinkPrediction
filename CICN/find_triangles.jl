...
function find_triangles_1(A::SparseMatrixCSC{T,Int64}) where T
    A = MatrixNetwork(A)
    #return find_triangles(MatrixNetwork(A));
    return A;
end

function find_triangles_2(A::MatrixNetwork)
    return find_triangles_3(A, true, true);
end
#println(find_triangles)
...
function find_triangles_3(A::MatrixNetwork,weighted::Bool,normalized::Bool)
    donorm = true
    usew = true
    if !normalized
        donorm = false
...
function find_triangles_phase2(donorm::Bool,rp::Vector{Int64},ci::Vector{Int64},
    ...
    #println("before reshape")
    all_triangles_2 = reshape(all_triangles,3,:)
    #println(all_triangles)
    return all_triangles_2
end
